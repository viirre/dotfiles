...
- Most common type should be first in multi-type docblocks:
  ...
  ```
- If only one parameter needs docblock, only add docblock for that parameter
...
// Multi-line ternary (PSR-12 style)
...
- A space after control structures:
  ```blade
  @if ($condition)
      Something
  @endif
...
- Policies use camelCase: `user()->can('editPost', ...)`
- Use CRUD words, but `view` instead of `show`

## Translations

- Use `__()` function over `@lang`, but use the `t()` helper for projects that use it
...
- Prefer to create concrete classes instead of mocks. Generally use mocks for classes that make HTTP-requests
- If the project has PHPUnit >= 10, use the `#[Test]` attribute instead of the `/** @test */` doc block.
- **Never create models in setUp for tests** - Use private helper methods instead to create models when needed by individual tests
